...
def Save(request, event_id):
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            ...
            messages.add_message(request, messages.SUCCESS, trans(u'Anmälan sparad'))
            return HttpResponseRedirect(reverse('event:view', args=(event_id,)))

    c = {'form': form, 'event': event}
    messages.add_message(request, messages.ERROR, trans(u'Anmälan misslyckades'))
...
def Remove(request, event_id, signup_id):
    ...
    if (u.title and u.title.styrelsePost) or u.is_superuser or (
            u == signup.member and event.deadline > datetime.utcnow().replace(tzinfo=pytz.utc)):
        signup.delete()
        delete_extra_fields(signup)
        messages.add_message(request, messages.SUCCESS, trans(u'Anmälan borttagen'))
    else:
        messages.add_message(request, messages.ERROR, trans(u'Du har tyvärr inte rättigheter att ta bort denna anmälan\
...
def Edit(request, signup_id):
    ...
    if request.method == 'POST':
        if (u.title and u.title.styrelsePost) or u.is_superuser or (
            ...
            messages.add_message(request, messages.SUCCESS, trans(u'Anmälan uppdaterad'))
            return HttpResponseRedirect(reverse('event:view', args=(signup.event_id,)))
        else:
            messages.add_message(request, messages.ERROR, trans(u'Du har tyvärr inte rättigheter att ta ändra denna anmälan,\
             vänligen kontakta styrelsen'))
